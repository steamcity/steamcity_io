...
const COUNTRIES_DATA = {
  "France": {
    ...
    studentNames: [
      "Antoine Dubois", "Sarah Moreau", "Lucas Martin", "Emma Leroy",
      "Paul Simon", "Léa Petit", "Hugo Durand", "Alice Robert",
      "Tom Garcia", "Chloé Bernard", "Nathan Fournier", "Manon Roux",
      "Louis Dupont", "Clara Girard", "Théo Laurent", "Inès Michel",
      "Maxime André", "Camille Lefèvre", "Arthur Thomas", "Zoé Faure"
    ]
  },
  "Belgium": {
    ...
    studentNames: [
      "Nicolas Janssen", "Louise Van Der Berg", "Alexandre Peeters",
      "Sophie Hendricks", "Maxime Claes", "Emma Willems", "Thomas De Vos",
      "Laura Mertens", "Antoine Dupuis", "Marie Leclercq", "Simon Jacobs",
      "Amélie Van Den Berg", "Julien Martens", "Charlotte Vermeulen"
    ]
  },
  "Spain": {
    language: "es",
    ...
    studentNames: [
      "Pablo García", "Sofía Rodríguez", "Diego Martínez", "Valentina López",
      "Mateo Sánchez", "Isabella Pérez", "Santiago González", "Camila Romero",
      "Nicolás Herrera", "Martina Ruiz", "Alejandro Torres", "Lucía Flores",
      "Gabriel Morales", "Victoria Castro", "Daniel Ortega", "Antonella Vargas"
    ]
  },
  "Italy": {
    language: "it",
    ...
    studentNames: [
      "Marco Rossi", "Giulia Bianchi", "Francesco Romano", "Sofia Esposito",
      "Alessandro Marino", "Chiara Ricci", "Matteo Greco", "Valentina Galli",
      "Leonardo Conti", "Francesca De Santis", "Davide Palmieri", "Martina Serra",
      "Gabriele Rizzo", "Aurora Lombardi", "Antonio Caruso", "Giorgia Ferri"
    ]
  },
  "Bulgaria": {
    ...
    studentNames: [
      "Viktor Petrov", "Elena Dimitrova", "Aleksandar Georgiev", "Maria Ivanova",
      "Dimitar Stoyanov", "Teodora Nikolova", "Hristo Vasilev", "Yoana Todorova",
      "Plamen Angelov", "Ralitsa Hristova", "Bozhidar Mihailov", "Desislava Petrova",
      "Atanas Dobrev", "Kristina Koleva", "Emil Yankov", "Silviya Stoycheva"
    ]
  }
};
...
const TRANSLATIONS = {
  fr: {
    ...
    experimentDescriptionTemplate: "Mesure et analyse dans le cadre du protocole {protocol} à {school}",
    hypotheses: {
      "Air Quality": [
  ...
  es: {
    ...
    experimentDescriptionTemplate: "Medición y análisis en el marco del protocolo {protocol} en {school}",
    hypotheses: {
      "Air Quality": [
  ...
  it: {
    ...
    experimentDescriptionTemplate: "Misurazione e analisi nel quadro del protocollo {protocol} presso {school}",
    hypotheses: {
      "Air Quality": [
  ...
  en: {
    ...
    experimentDescriptionTemplate: "Measurement and analysis within the {protocol} protocol at {school}",
    hypotheses: {
      "Air Quality": [
...
function generateCoherentExperiment(protocol, country, city, school, studentName, index) {
  ...
  const categoryHypotheses = translations.hypotheses[protocol.category] || translations.hypotheses["Data Analysis"];
  ...
  const categoryMethodologies = translations.methodologies[protocol.category] || translations.methodologies["Data Analysis"];
  const methodology = categoryMethodologies[Math.floor(Math.random() * categoryMethodologies.length)];
...
function generateAllExperiments() {
  ...
  DETAILED_PROTOCOLS.forEach(protocol => {
    ...
    for (let i = 0; i < experimentsPerProtocol; i++) {
      ...
      const studentName = countryData.studentNames[Math.floor(Math.random() * countryData.studentNames.length)];

      const experiment = generateCoherentExperiment(protocol, country, city.name, school, studentName, index);
