...
const getUniqueSensors = async (req, res) => {
    try {
        ...
        filteredData.forEach(measurement => {
            ...
            if (
                !sensorGroups[key].lastTimestamp ||
                new Date(measurement.timestamp) > new Date(sensorGroups[key].lastTimestamp)
            ) {
    ...
    try {
        ...
        if (experimentId) {
            filteredMeasurements = filteredMeasurements.filter(
                m => m.experiment_id === experimentId
            )
        }
        ...
        // Limit results
        const limitedMeasurements = limit
            ? filteredMeasurements.slice(0, parseInt(limit))
