...
const getClusterProtocols = async (req, res) => {
    try {
        ...
        const clusterProtocols = protocols.filter(
            p => p.primaryCluster === parseInt(id) || p.secondaryClusters.includes(parseInt(id))
        )
    ...
    try {
        ...
        if (cluster) {
            ...
            protocols = protocols.filter(
                p => p.primaryCluster === clusterId || p.secondaryClusters.includes(clusterId)
            )
        }
...
const getProtocolById = async (req, res) => {
    try {
        ...
        const secondaryClusters = clusters.filter(c => protocol.secondaryClusters.includes(c.id))
...
const searchProtocols = async (req, res) => {
    try {
        ...
        let filteredProtocols = protocols.filter(
            p =>
                p.name.toLowerCase().includes(searchLower) ||
                p.description?.toLowerCase().includes(searchLower) ||
                p.keywords.some(k => k.toLowerCase().includes(searchLower))
